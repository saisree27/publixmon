...
export async function apiBackend(){
  ...
  let productNames=[]
      try {
        let date = new Date()
        let res = await fetch("https://gateway-staging.ncrcloud.com/transaction-document/transaction-documents/find", {
            method:'POST',
            headers: {
        ...
        await Promise.all(tlogIDs.map(async function(x){
          let date= new Date()
          let res = await fetch("https://gateway-staging.ncrcloud.com/transaction-document/transaction-documents/"+x, {
              method:'GET',
              headers: {
                  'Content-Type': 'application/json',
    ...
    let NFTProduct=productNames[Math.floor(Math.random() * productNames.length)]
    NFTProduct = NFTProduct.replace(/\s/g, '');
    return "https://image-stg-cdn.ncrcloud.com/test-drive-da9200603a984026a1e16-dce174a0cb0a4bc4b8cd7e675b6aa570/image-container/"+NFTProduct+".jpeg"
}